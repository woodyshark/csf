...
<script language="JavaScript">
	...
	function changegood(aa){
		if (aa.options[aa.selectedIndex].value == "") {
			...
			form1.action="material_addgood.jsp?good_id="+bb;
			form1.submit();
		}
	}
	function qua_unit2(){
...
<%
		ResultSet rs1 = stmt.executeQuery("select g.good_id,g.name from good g,good_type t where g.good_type_id=t.good_type_id and g.trans<>'C' order by t.name,g.name");
		while(rs1.next()){
...
<%
		...
		float price2=0;String price_unit2="";float sale_low2=0;float sale_high2=0;String price_unit_name2="";
		...
		if(good_id.equals("")){
			...
			total_unit_high = rs.getFloat(6);
			 rs = stmt.executeQuery("select d.price,d.price_unit,d.sale_low,d.sale_high,m.mate_date from material_detail d,material m where m.mate_no=d.mate_no and m.year=d.year and d.good_id='"+good_id+"' and m.client_id='"+client_id+"' order by mate_date desc,m.mate_no desc");
			if(rs.next()){
					price = rs.getFloat(1);
			...
			rs = stmt.executeQuery("select price,price_unit,sale_low,sale_high from good where good_id='"+good_id+"'");	
			if(rs.next()){
				price2 = rs.getFloat(1);
				price_unit2 = rs.getString(2);if(price_unit2==null){price_unit2="";}
				sale_low2 = rs.getFloat(3);
				sale_high2 = rs.getFloat(4);
			}
			if(price_unit2.equals("1")){
				price_unit_name2 = unit_low;
			}else{
				price_unit_name2 = unit_high;
			}
			rs.close();
		}
%>
...
%>
			</select> ราคาที่ตั้งไว้ <%=convertcomma.dtoa(Float.toString(price2),"#,##0.00")%> บาท ต่อ <%=price_unit_name2%>
		</td>
	</tr>
	<tr class="tr1">
		<td align="right" class="h1">(ราคาขายต่อ<%=unit_low%> *<input type="hidden" name="unit_low" value="<%=unit_low%>"></td>
		<td><input type="text" class="txt" size="10" maxlength="15" name="sale_low" value="<%=sale_low%>" > ราคาที่ตั้งไว้ <%=convertcomma.dtoa(Float.toString(sale_low2),"#,##0.00")%> บาท)</td>
	</tr>
<%
		if(unit_high.equals("")){
%>
...
%>
	<tr class="tr1">
		...
		<td><input type="text" class="txt" size="10" maxlength="15" name="sale_high" value="<%=sale_high%>"> <%=rate%> <%=unit_low%> ต่อ 1 <%=unit_high%> ราคาที่ตั้งไว้ <%=convertcomma.dtoa(Float.toString(sale_high2),"#,##0.00")%> บาท)</td>
	</tr>
<%
		}	
%>
