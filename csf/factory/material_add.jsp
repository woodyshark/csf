...
<%   
	...
	try{
		...
		String mate_date = request.getParameter("mate_date");if(mate_date==null){mate_date="";}
		String mate_date_show = request.getParameter("mate_date_show");if(mate_date_show==null){mate_date_show="";}
...
<script language="JavaScript">
	function addgood(){	
		if (!hasInput(form1.mate_date.value)){			
			var redObj = new Array(form1.mate_date_show);
		...
		if (document.form1.client_id.options[document.form1.client_id.selectedIndex].value == "") {
			if (!hasInput(form1.client_id.value)){			
				...
				setErrorColor(redObj);			
				alert("กรุณากรอกชื่อผู้ผลิต");
				form1.client_id.focus();
				return;
			}
		}
		data=document.form1.client_id.options[document.form1.client_id.selectedIndex].value;
		var dialogUrl = "material_addgood.jsp?client_id="+data;
		var material_addgood = window.open(dialogUrl,"material_addgood","toolbar=0, location=0, directories=0, status=0, menubar=0, scrollbars=0, height=280, width=450,resizable=0");
		material_addgood.focus();
	}
	function adddetail(good_id,good_name,price,sale_low,sale_high,quantity,amount,price_unit,price_unit_name,unit_low,unit_high) {
		...
		form1.mate_date_show.disabled=false;
		form1.client_id.disabled=false;
		form1.action="material_add.jsp?good_id="+good_id+"&good_name="+good_name+"&price="+price+"&sale_low="+sale_low+"&sale_high="+sale_high+"&quantity="+quantity+"&amount="+amount+"&price_unit="+price_unit+"&price_unit_name="+price_unit_name+"&unit_low="+unit_low+"&unit_high="+unit_high;
		form1.submit();
	}
	function sm(){
		if (!hasInput(form1.mate_date.value)){			
		...
		form1.action="material_adddata.jsp";
		form1.submit();
	}
	function del(){  
	...
	function setCalendarInfo(strRtnId, strDate){	
		if (strRtnId == 1) {	
			form1.mate_date_show.value = strDate.substring(6,8)+"/"+strDate.substring(4,6)+"/"+strDate.substring(0,4); 
			form1.mate_date.value = strDate; 
		}else if (strRtnId == 2) {	
...
<body>
<font  class="f1" >เพิ่มใบสั่งซื้อวัตถุดิบ-สินค้า</font><br>
<form name="form1" method="post"  >
<table  width="100%"  cellpadding="3" cellspacing="2" border="0">
	<tr class="tr1">
		<td align="right" class="h1">ใบสั่งซื้อลงวันที่ *</td>
		<td><input type="text" class="txt" size="10" maxlength="10" name="mate_date_show" disabled value="<%=mate_date_show%>"><input type="hidden" name="mate_date"  value="<%=mate_date%>">  <input type="button" name="date_butt1" class="butt" value="..." onClick="getCalendarInfo(1,'opener')"> </td>
	</tr>
	<tr class="tr2">
...
<%
		ResultSet rs1 = stmt.executeQuery("select client_id,company_name,name,city from client where type='S' order by city,company_name,name");
		while(rs1.next()){
...
%>
	<tr class="<%=background%>" ><input type="hidden" name="price_unit_name"  value="<%=price_unit_name[k]%>"><input type="hidden" name="unit_low"  value="<%=unit_low[k]%>"><input type="hidden" name="unit_high"  value="<%=unit_high[k]%>">
		...
		<td align="right" ><input type="hidden" name="price"  value="<%=price[k]%>"><%=convertcomma.dtoa(price[k],"#,##0.00")%></td>
...
<table  width="100%"  cellpadding="3" cellspacing="2" border="0">
	...
	<tr class="tr2">
		...
		<td><input type="text" class="txt" size="10" maxlength="10" name="pay_date_show" disabled><input type="hidden" name="pay_date">  <input type="button" name="date_butt2" class="butt" value="..." onClick="getCalendarInfo(2,'opener')" disabled></td>
	</tr>
	<tr class="tr1">
