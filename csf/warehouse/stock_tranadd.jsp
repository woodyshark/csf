		...
		if(request.getParameter("stockin") != null){
			...
			if	(stockin != null && stockin.length > 0) {
				for(int k=0; k<stockin.length; k++){
					try{
						stmt.executeUpdate("update trans set stock='Y' where concat(trim(trans_no),'/',year) = '"+stockin[k]+"'");
							rs = stmt.executeQuery("select d.good_id,d.quantity,d.price_unit,g.unit_low,g.unit_high,g.rate from trans_detail d,good g where d.good_id=g.good_id and concat(trim(d.trans_no),'/',d.year)  = '"+stockin[k]+"'");	
...
<head><meta equiv="Content-Type" content="text/html; charset=TIS-620">
	<title>Untitled</title>
	 <script language="JavaScript">
		window.location="stock_tran.jsp";
 	 </script>
</head>
<body>
</body>
