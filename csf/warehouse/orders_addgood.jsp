...
<%
		ResultSet rs1 = stmt.executeQuery("(select  (d .good_id), g.name,t.name from material_detail d, good g,good_type t where d .quantity_stock <> 0 and d .good_id = g.good_id and g.trans = 'n' and g.good_type_id=t.good_type_id limit 1)union (select  (d .good_id), g.name,t.name from trans_detail d, good g,good_type t where d .quantity_stock <> 0 and d .good_id = g.good_id and g.trans = 'n' and g.good_type_id=t.good_type_id order by t.name,g.name limit 1)");
		while(rs1.next()){
