...
<%
	...
	try{
		...
		try{
			...
			String where2 = fromtodate.where(start_dd,start_mm,start_yy,end_dd,end_mm,end_yy,"o.orders_date");
			ResultSet rs1 = stmt.executeQuery("select count(*) from orders o,client c,employee e where o.client_id=c.client_id and o.employee_id=e.employee_id "+where1+ where2);	
			rs1.next();
			counter = rs1.getString(1);
			rs1.close();
			rs = stmt.executeQuery("select o.orders_no,o.year,o.orders_date,c.company_name,c.name,e.first_name,e.last_name,o.total_discount from orders o,client c,employee e where o.client_id=c.client_id and o.employee_id=e.employee_id "+where1+where2+" order by o.year desc,o.orders_no desc");	
...
<head><meta equiv="Content-Type" content="text/html; charset=TIS-620">
	...
	function gopage(pages) {
		document.form1.action="orders_search.jsp?pages="+pages;
		document.form1.submit();
	}
	function detail(orders_no,year){
		window.location="orders_detail.jsp?orders_no="+orders_no+"&year="+year;
	}
	</script>
</head>
<body> 
<font  class="f1">ค้นหาใบขายสินค้า</font>
<form action="orders_search.jsp" name="form1" method="post">
